...
# Resolve project root (pipeline/ -> project root)
...
logger.add(str(PROJECT_ROOT / "logs" / "archive_processing_{time}.log"), rotation="10 MB", level="DEBUG")
...
async def process_loose_images():
    ...
    # Save report
    report_path = PROJECT_ROOT / "output" / "ocr" / "reports" / "images_processing_report.json"
...
async def process_kheel_materials():
    ...
    kheel_base = PROJECT_ROOT / "raw" / "scans" / "Kheel Center"
    ...
    # Save report
    report_path = PROJECT_ROOT / "output" / "ocr" / "reports" / "kheel_processing_report.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
...
async def process_nys_archives():
    ...
    nys_base = PROJECT_ROOT / "raw" / "scans" / "NYS Archives"
    ...
    # Save report
    report_path = PROJECT_ROOT / "output" / "ocr" / "reports" / "nys_archives_processing_report.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
