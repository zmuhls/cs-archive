#!/usr/bin/env python3
"""
Process South-Kortright-Roll-13.pdf using Qwen VL Plus model.

This script processes the 43-page South Kortright PDF (handwritten cursive) with:
- Verbose logging showing progress for each page
- Stamp noise filtering (removes duplicate microfilm stamps)
- qwen/qwen-vl-plus model with handwritten-specific prompts
- Optimized for 19th century cursive handwriting

Run with: python scripts/process_south_kortright.py
"""
...
logger.add(
    str(PROJECT_ROOT / "logs" / "south_kortright_processing_{time}.log"),
    rotation="10 MB",
    level="DEBUG",
...
async def process_south_kortright(run_id: Optional[str] = None):
    ...
    pdf_path = PROJECT_ROOT / "raw" / "scans" / "NYS Archives" / "A4645" / "South-Kortright-Roll-13.pdf"
    ...
    try:
        ...
        total_pages = 43  # Known count

    print(f"PDF: {pdf_path.name}")
    print(f"Total pages: {total_pages}")
    print(f"Document type: Handwritten cursive (meeting minutes)")
    print(f"Estimated time: {format_duration(total_pages * 15)}  (assuming ~15s per page for handwriting)")
    print()
    print("-" * 70)
    print()
    ...
    CHUNK_SIZE = 5  # Smaller chunks for handwritten documents
        ...
        for chunk_start in range(1, total_pages + 1, CHUNK_SIZE):
            ...
            for i, image in enumerate(images):
                ...
                try:
                    ...
                    result["series"] = "A4645"
                    result["document_nature"] = "handwritten_cursive"
                    ...
                    results.append({
                        ...
                        "series": "A4645",
                        "document_nature": "handwritten_cursive",
                        "error": str(e),
    ...
    report = {
        "source": str(pdf_path),
        "model": ocr.model,
        "document_type": "handwritten_cursive",
    ...
    report_path = PROJECT_ROOT / "output" / "ocr" / "reports" / "south_kortright_processing_report.json"
    ...
    print(f"  Output: output/ocr/text/South-Kortright-Roll-13_*.txt")
    print()
    print("=" * 70)
