#!/usr/bin/env python3
"""
Process District-Notecard-Records.pdf using Qwen VL Plus model.

This script processes the 63-page district notecard PDF with:
...
- Structured notecard format (1-2 cards per page)

Run with: python scripts/process_notecards.py
"""
...
logger.add(
    str(PROJECT_ROOT / "logs" / "notecards_processing_{time}.log"),
    rotation="10 MB",
    level="DEBUG",
...
async def process_notecards(run_id: Optional[str] = None):
    """Process District-Notecard-Records.pdf with verbose logging.
    ...
    print("=" * 70)
    print("DISTRICT NOTECARD RECORDS PROCESSING")
    print("Model: qwen/qwen-vl-plus (photographed notecards)")
    print("Stamp filtering: DISABLED (not microfilm)")
    print("=" * 70)
    print()

    # Initialize OCR with run_id for versioning
    # Disable stamp filtering - these are photographed cards, not microfilm scans
    ocr = QwenVLOCR(config_path="ocr_config.yaml", run_id=run_id, enable_stamp_filtering=False)
    ocr.model = "qwen/qwen-vl-plus"
    logger.info(f"Initialized OCR with model: {ocr.model}")
    logger.info(f"Run ID: {ocr.run_id}")
    logger.info("Document type: NOTECARD (photographed index cards)")
    logger.info("Stamp noise filtering: DISABLED (photographed cards, not microfilm)")

    pdf_path = PROJECT_ROOT / "raw" / "scans" / "NYS Archives" / "B0594" / "District-Notecard-Records.pdf"
    ...
    try:
        ...
        total_pages = 63  # Known count

    print(f"PDF: {pdf_path.name}")
    print(f"Total pages: {total_pages}")
    print(f"Format: 1-2 index cards per page (administrative timelines)")
    print(f"Estimated time: {format_duration(total_pages * 10)}  (assuming ~10s per page)")
    print()
    print("-" * 70)
    print()
    ...
    CHUNK_SIZE = 5  # Smaller chunks for notecard processing
    ...
    try:
        ...
        for chunk_start in range(1, total_pages + 1, CHUNK_SIZE):
            ...
            for i, image in enumerate(images):
                ...
                # Process the image with NOTECARD document type
                try:
                    logger.info(f"Page {page_num}/{total_pages}: Sending to API (notecard mode)...")
                    ...
                    result["series"] = "B0594"
                    result["document_nature"] = "administrative_notecard"
                    ...
                    results.append({
                        ...
                        "series": "B0594",
                        "document_nature": "administrative_notecard",
                        "error": str(e),
    ...
    report = {
        "source": str(pdf_path),
        "model": ocr.model,
        "document_type": "administrative_notecard",
    ...
    report_path = PROJECT_ROOT / "output" / "ocr" / "reports" / "notecards_processing_report.json"
    ...
    print(f"  Output: output/ocr/text/District-Notecard-Records_*.txt")
    print()
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process District Notecard Records PDF")
    parser.add_argument(
        "--run-id",
        type=str,
        default=None,
    ...
    asyncio.run(process_notecards(run_id=args.run_id))
